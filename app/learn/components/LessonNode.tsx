...
export const LessonNode: React.FC<LessonNodeProps> = ({ 
  ...
  const handleStartLesson = () => {
    if (lessonId) {
      router.push(`/lesson?baihoc=${lessonId}`);
    }
    setShowModal(false);
  };
  ...
  return (
    <>
          ...
          {!showModal && type === 'lesson' && status === 'active' && (
            <div
              className="absolute -top-8 left-1/2 -translate-x-1/2 bg-white px-4 py-1.5 
                ...
                before:border-l-8 before:border-r-8 before:border-t-[11px]
                before:border-l-transparent before:border-r-transparent before:border-t-gray-500
                after:content-[''] after:absolute after:left-1/2 after:-translate-x-1/2 
                after:top-[calc(100%-1px)] after:w-0 after:h-0
                after:border-l-8 after:border-r-8 after:border-t-[10px]
      ...
      {showModal && (
        <div className="fixed inset-0 z-[9999]" onClick={() => setShowModal(false)}>
          <div
            className="fixed z-[9999] -translate-x-1/2 animate-fadeIn rounded-2xl p-6 w-96 shadow-2xl"
            style={{
              top: modalPos.top,
              left: modalPos.left,
          ...
          >
            <div
              className="absolute -top-2 left-1/2 -translate-x-1/2 w-0 h-0 border-l-8 border-r-8 border-b-[10px] border-l-transparent border-r-transparent"
              style={{
            ...
            <button
              ...
              className="w-full bg-white text-lg font-bold py-4 rounded-xl cursor-pointer transition-all duration-200 hover:brightness-110"
              style={{
                color: colorToHex(color)
              }}
            >
