...
export default function QuizPage() {
  ...
  const AddPoints = async (totalPoints: number, userId: string): Promise<boolean> => {
    try {
      const response = await fetch(
        `http://127.0.0.1:8000/api/users/${totalPoints}/${userId}/${baiHocId}`,
        { method: 'PUT' }
      );
      ...
      if (!result.success) {
        alert(result.message || 'Không thể cập nhật điểm. Bài học này có thể đã hoàn thành.');
        return false;
      }
  ...
  const canCheck = () => {
    ...
    if (
      ...
      currentQuestion.LoaiCauHoi === 'dientu' ||
